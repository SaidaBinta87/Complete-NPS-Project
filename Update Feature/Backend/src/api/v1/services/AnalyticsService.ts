...
export const getSurveyStatusData = async (): Promise<{ surveySentCount: number, surveyCompleteCount: number, surveyResentCount: number }> => {
  try {
    const surveySentCount = await UploadData.countDocuments({ status: 'Survey Sent' });
    const surveyCompleteCount = await UploadData.countDocuments({ status: 'Survey Completed' });
    const surveyResentCount = await UploadData.countDocuments({ status: 'Survey Resent' });
...
export const getRecentFiles = async (userId: string): Promise<{ today: any[], yesterday: any[] }> => {
    try {
      ...
      const todayFiles = await Uploads.find({ userId: userId, createdAt: { $gte: today, $lt: new Date() } });
      const yesterdayFiles = await Uploads.find({ userId: userId, createdAt: { $gte: yesterday, $lt: today } });
