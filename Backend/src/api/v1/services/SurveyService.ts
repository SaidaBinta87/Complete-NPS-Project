...
export class SurveyService {
  static async generateUniqueLinksAndSave(fileId: string, emailSubject: string, emailText: string): Promise<string[]> {
    try {
      ...
      for (let i = 1; i < recipients.length; i++) {
        ...
        const fileData: Partial<IFileData> = {
        ...
        await FileData.create(fileData);
  ...
  static async resendSurveyToUser(uniqueId: string): Promise<void> {
    try {
      ...
      const userToResend = await FileData.findOne({ uniqueId, status: 'Survey Sent' });
      ...
      await FileData.findByIdAndUpdate(userToResend._id, { status: 'Survey Resent' });
    } catch (error) {
  ...
  static async searchFileData(fileId: string, searchText: string): Promise<IFileData[]> {
    try {
